...
As digital objects take shape, they require more than stable files and durable storage. They need structured information that makes them discoverable and intelligible beyond the immediate research context. This is the role of metadata.

## What Is Metadata in Research Data Management?

In the context of Research Data Management, metadata is structured information that describes, explains, and contextualises a digital object such as a dataset, document, image, or software file. It acts as a layer of documentation that makes a digital object findable, understandable, and reusable, both now and in the future.

Metadata typically captures:

* Content information — what the object contains
* Provenance — how, when, and by whom it was created or modified
...
digraph do_mesh {
    ...
    subgraph cluster_DOID {
        ...
        DOID_HD  [label="Heart disease"];
        DOID_VD  [label="Vascular disease"];
        DOID_Ath [label="Atherosclerosis"];
        DOID_CA  [label="Coronary artery"];
        DOID_CAD [label="Coronary artery disease"];
        ...
        DOID_CAD -> DOID_CA  [label="occurs_in", style=dotted];
    ...
    subgraph cluster_MeSH {
        ...
        MeSH_CVD [label="Cardiovascular Diseases"];
        MeSH_HD  [label="Heart Diseases"];
        MeSH_CAD [label="Coronary Disease"];
...
```
@fig-mesh-vs-do highlights how controlled vocabularies and ontologies differ in the way they structure knowledge. In MeSH, “Coronary Disease” appears in a straightforward hierarchical tree under “Heart Diseases,” reflecting its role as a curated list of terms designed for consistent indexing and retrieval. The Disease Ontology, in contrast, represents “Coronary artery disease” as a formal class with multiple parents and additional relations to anatomical and pathological entities. These relations express biological context such as the involvement of the coronary artery or the role of atherosclerosis and support machine‑interpretable reasoning.

Together, the two representations show how controlled vocabularies provide stable terminology, while ontologies add semantic structure that enables richer connections between concepts and datasets.

While ontologies describe the meaning of concepts and the relationships between them, metadata schemas address a different question: which pieces of information should be captured when describing a digital object.
...
Schemas define the structure of metadata, but researchers rarely interact with schemas directly. Templates translate these definitions into practical forms and interfaces that guide real‑world data entry.
...
It is most useful when metadata needs to express relationships, connect to external vocabularies, or grow without restructuring. Key–value formats work well for simple descriptions, but Linked Data supports richer semantics, interoperability, and long‑term extensibility.
This richer structure allows metadata to be connected across systems, enabling more powerful discovery and integration.

This would strengthen the mot
...
Together, these layers create rich, FAIR, and interoperable metadata. With them in place, metadata becomes both meaningful within its discipline and interoperable across the wider research landscape.
